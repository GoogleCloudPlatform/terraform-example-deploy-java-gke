...
function configure() {
  ...
  grep "${JGROUP_BUCKET_NAME}" /usr/local/tomcat/webapps/ROOT/WEB-INF/observation/remote/jgroups/tcp.xml > /dev/null && echo "Replace Jgroup Bucket Name Complated ! "
  sed -i "s#ACCESS_KEY#${ACCESS_KEY}#g" /usr/local/tomcat/webapps/ROOT/WEB-INF/observation/remote/jgroups/tcp.xml
  grep "${ACCESS_KEY}" /usr/local/tomcat/webapps/ROOT/WEB-INF/observation/remote/jgroups/tcp.xml > /dev/null && echo "Replace Access Key Complated ! "
  sed -i "s#SECRET_KEY#${SECRET_KEY}#g" /usr/local/tomcat/webapps/ROOT/WEB-INF/observation/remote/jgroups/tcp.xml
  grep "${SECRET_KEY}" /usr/local/tomcat/webapps/ROOT/WEB-INF/observation/remote/jgroups/tcp.xml > /dev/null && echo "Replace SECRET Access Key Complated ! "
}
